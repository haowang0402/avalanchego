...
if [[ $# -eq 2 ]]; then
    ...
    if [[ ! -d "$coreth_path" ]]; then
        go get "github.com/haowang0402/coreth@$coreth_version"
    fi
else
...
cd "$coreth_path"
go build -ldflags "-X github.com/haowang0402/coreth/plugin/evm.Version=$coreth_version $static_ld_flags" -o "$evm_path" "plugin/"*.go
cd "$AVALANCHE_PATH"
